...
const defaultPath = path.resolve(path.dirname(fileURLToPath(import.meta.url)), 'templates.json');
...
export const createTemplatesStore = (filePath = defaultPath) => ({
  async getAll() {
  ...
  async save(templates) {
    ...
    await writeStore(filePath, templates);
    return templates;
  }
});
